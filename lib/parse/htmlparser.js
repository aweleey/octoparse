 ...
 * 
 */
import { startTag, endTag, attr, empty, block, inline, closeSelf, fillAttrs, special } from './constant/index';
  ...
  while (html) {
    ...
    if (!stack.last() || !special[stack.last()]) {
      ...
      if (html.indexOf("<!--") == 0) {
        ...
        match = html.match(startTag); // console.log('match的是', match);

        if (match) {
          html = html.substring(match[0].length); // console.log('replace前的是', match[0])	

          match[0].replace(startTag, parseStartTag); // console.log('replace后的是', match[0])
  ...
  function parseStartTag(tag, tagName, rest, unary) {
    ...
    if (handler.start) {
      ...
      rest.replace(attr, function (match, name) {
        ...
        var value = arguments[2] ? arguments[2] : arguments[3] ? arguments[3] : arguments[4] ? arguments[4] : fillAttrs[name] ? name : ""; // console.log('value是', value);
